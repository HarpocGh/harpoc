...
describe("createSecret", () => {
  ...
  it("accepts bearer injection config", () => {
    const result = manager.createSecret({
      name: "bearer-inj",
      type: "api_key",
  ...
  it("accepts header injection config", () => {
    const result = manager.createSecret({
      name: "header-inj",
      type: "api_key",
...
describe("resolveHandle", () => {
  ...
  it("resolves active secret when revoked secret has same name", () => {
    // Create, then revoke
    manager.createSecret({
      name: "reused-name",
      type: "api_key",
    ...
    manager.revokeSecret("secret://reused-name");
    ...
    const secret = manager.resolveHandle("secret://reused-name");
    expect(secret.status).toBe("active");

    const value = manager.getSecretValue("secret://reused-name");
    expect(Buffer.from(value).toString()).toBe("new");
  });

  it("still resolves a single revoked secret", () => {
    manager.createSecret({
      name: "only-revoked",
      type: "api_key",
      value: new Uint8Array(Buffer.from("v")),
    });
    manager.revokeSecret("secret://only-revoked");

    const secret = manager.resolveHandle("secret://only-revoked");
